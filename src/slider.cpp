...
lv_area_t operator-(lv_area_t const& coord1, lv_point_t const& offset)
{
   return lv_area_t
   {
      coord1.x1 - offset.x, coord1.y1 - offset.y, coord1.x2 - offset.x, coord1.y2 - offset.y,
   };
}
...
void Slider::draw(MiniCanvas* canvas)
{
   ...
   lv_area_t adjustedCoords = coords - lv_point_t{canvas->img.obj.coords.x1, canvas->img.obj.coords.y1};
   ...
   std::int32_t sliderWidth = ((adjustedCoords.x2 - adjustedCoords.x1) * percent) / 100;
