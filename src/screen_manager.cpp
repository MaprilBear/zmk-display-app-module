...
ScreenManager::ScreenManager() {
  ...
  lv_canvas_t *lvCanvas =
      reinterpret_cast<lv_canvas_t *>(lv_canvas_create(lv_scr_act()));
  std::memcpy(static_cast<void *>(&miniCanvas1), lvCanvas, sizeof(lv_canvas_t));
  miniCanvas1.canvasBuffer = image_buffer;
  lv_canvas_set_buffer(reinterpret_cast<lv_obj_t *>(&miniCanvas1),
                       miniCanvas1.canvasBuffer, SCREEN_WIDTH,
  ...
  miniCanvas2.canvasBuffer =
      &image_buffer[SCREEN_WIDTH * (SCREEN_HEIGHT / 2) * 2];
  lv_canvas_set_buffer(reinterpret_cast<lv_obj_t *>(&miniCanvas2),
                       miniCanvas2.canvasBuffer, SCREEN_WIDTH,
                       SCREEN_HEIGHT / 2, LV_COLOR_FORMAT_RGB565);
...
void ScreenManager::flush(MiniCanvas *canvas) {
  // LOG_INF("Flushing...");
  display_write(display_dev, canvas->img.obj.coords.x1,
                canvas->img.obj.coords.y1, &display_desc, canvas->canvasBuffer);
  // LOG_INF("Flushing complete!");
}

K_SEM_DEFINE(displayRefreshSema, 1, 1)

void displayRefresh(struct k_timer *dummy) { k_sem_give(&displayRefreshSema); }
...
K_THREAD_DEFINE(flushing_thread, 4048, ScreenManager::flushLoop, NULL, NULL,
                NULL, 3, 0, 0);